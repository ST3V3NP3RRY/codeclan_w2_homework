...
class TestRoom(unittest.TestCase):
    ...
    def test_find_song_by_title__song_found(self):
    ...
    def test_find_song_by_title__song_not_found(self):
        song = Song("Welcome to the Jungle", "Guns 'n' Roses", "Rock")
        self.room.add_song_to_room(song)
        result = self.room.find_song_by_title("Back in Black")
        self.assertEqual(None, result)

    def test_guest_can_afford_fee__sufficent_funds(self):
        guest = self.guest3
        can_afford_fee = self.room.customer_can_afford_fee(guest)
        self.assertEqual(True, can_afford_fee)

    def test_guest_can_afford_fee__insufficent_funds(self):
        guest = self.guest2
        can_afford_fee = self.room.customer_can_afford_fee(guest)
        self.assertEqual(False, can_afford_fee)

    def test_find_guest_favourite_song__song_found(self):
